...
def escape_color(color: str) -> str:
...
if __name__ == '__main__':
    ...
    output = [
        raw_line.decode('utf-8').strip() for raw_line in p.communicate()[0].splitlines()
    ]

    # This thing outputs structured but inconsistent json-like content. These fields are simple "key"=value
    def parse_float(property_name: str) -> float:
    ...
    print(
        escape_color('green')
        + (FULL_ICON * fill_count)
        + escape_color('red')
        + (EMPTY_ICON * empty_count)
        + escape_color('reset')
    )
