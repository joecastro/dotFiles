...
class EC2InstanceStatusSummary:
    def __init__(
        self, instance_id: str, instance_status: str, system_status: str
    ) -> None:
        self.instance_id = instance_id
...
WaiterName = Literal[
    'instance_running', 'volume_available', 'volume_in_use', 'volume_deleted'
]
...
class EC2Service:
    ...
    def instance_status_summary(self, instance_id: str) -> EC2InstanceStatusSummary:
        ...
        inst_value = (
            instance_status.get('Status') if isinstance(instance_status, dict) else None
        )
        sys_value = (
            system_status.get('Status') if isinstance(system_status, dict) else None
        )
    ...
    def describe_volumes(
        self, volume_ids: Optional[Sequence[str]] = None
    ) -> list[EC2VolumeDescription]:
        if volume_ids:
...
class WorkstationMetadata:
    ...
    def load(cls) -> Optional[WorkstationMetadata]:
        ...
        try:
            ...
            raise ValueError(
                f'Metadata file {cls.META_FILE} is not valid JSON'
            ) from exc
...
class WorkstationManager:
    ...
    def create(self) -> None:
        ...
        if (
            ...
            or summary.instance_status != 'ok'
            or summary.system_status != 'ok'
        ):
            print(
    ...
    def teardown(self) -> None:
        ...
        try:
            ...
            print(
                f'Warning: failed to terminate instance: {_format_client_error(exc)}',
                file=sys.stderr,
            )
        ...
        if metadata.volume_id:
            try:
                ...
                print(
                    f'Warning: failed to delete volume: {_format_client_error(exc)}',
                    file=sys.stderr,
                )
    ...
    def start(self) -> None:
        ...
        if state == 'terminated':
            print(
                f'Instance {metadata.instance_id} is terminated; re-run create',
                file=sys.stderr,
            )
            sys.exit(1)
        ...
        if state == 'stopped':
            ...
            print(
                f"Instance {metadata.instance_id} in state '{state}', waiting until running..."
            )
            self.ec2.get_waiter('instance_running').wait(
                InstanceIds=[metadata.instance_id]
            )
    ...
    def list_volumes(self) -> None:
        ...
        for volume in volumes:
            ...
            print(
                f'  {vol_id}  {size_display}  {state}  {attachment_info}  Name={name}{suffix}'
            )
    ...
    def add_volume(
        ...
        *,
        ...
        print(
            f'Created volume {volume_id} ({size_gib} GiB) in {instance.availability_zone}'
        )
        try:
            ...
            print(
                f"Warning: volume {volume_id} did not reach 'available' state: {exc}",
                file=sys.stderr,
            )
        ...
        try:
            ...
            print(
                f'Volume {volume_id} created but attach failed: {_format_client_error(exc)}',
                file=sys.stderr,
            )
        ...
        try:
            ...
            print(
                f'Warning: unable to confirm attachment for {volume_id}: {exc}',
                file=sys.stderr,
            )
    ...
    def detach_volume(self, volume_id: str, *, force: bool) -> None:
        ...
        if not attachments:
            print(
                f'Volume {volume_id} is not attached to instance {metadata.instance_id}.'
            )
            return
        ...
        try:
            ...
            print(
                f'Warning: unable to confirm detach for {volume_id}: {exc}',
                file=sys.stderr,
            )
            return
    ...
    def destroy_volume(self, volume_id: str, *, force_detach: bool) -> None:
        ...
        if attachments:
            ...
            if other_instances:
                ...
                print(
                    'Detach the volume from those instances in the AWS console before retrying.',
                    file=sys.stderr,
                )
                sys.exit(1)
        ...
        try:
            ...
            print(
                f'Warning: unable to confirm deletion for {volume_id}: {exc}',
                file=sys.stderr,
            )
    ...
    def _derive_public_key(self, private_key_path: Path) -> str:
        ...
        if not derived:
            raise ValueError(
                f'ssh-keygen returned no public key for {private_key_path}'
            )
        return derived
...
PUBKEY
    ...
    def _create_and_attach_volume(self, instance_id: str) -> str:
        ...
        try:
            self.ec2.attach_volume(
                Device='/dev/xvdf', InstanceId=instance_id, VolumeId=volume_id
            )
        except ClientError as exc:
            print(
                f'Volume {volume_id} created but attach failed: {_format_client_error(exc)}',
    ...
    def _print_connect_info(
        self, public_ip: Optional[str], public_dns: Optional[str]
    ) -> None:
        ...
        if address:
            ...
            print(
                'Instance running but no public network address assigned yet. Check the AWS console for networking status.'
            )
    ...
    def _ensure_ssh_access(self, instance_id: str) -> None:
        try:
            ...
            print(
                f'Warning: unable to inspect instance {instance_id} for security groups: {exc}',
                file=sys.stderr,
            )
            return
        ...
        for sg in instance.security_groups:
            try:
                ...
                if added:
                    print(
                        f'Authorized SSH (22/tcp) from {cidr} on security group {sg.id}.'
                    )
            except ClientError as exc:
                print(
    ...
    def wait_for_instance_available(self, instance_id: str) -> EC2InstanceStatusSummary:
        ...
        def predicate() -> bool:
            ...
            instance = self.ec2.describe_instance(instance_id)
            print(
                f'  status checks for {summary.instance_id}: state={instance.state}, status={summary.instance_status}, system_status={summary.system_status}'
            )
    ...
    def _print_config_summary(self) -> None:
        cfg = self.config
        sg_entries = [
            part.strip() for part in cfg.security_group.split(',') if part.strip()
        ]
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument(
        '--profile', default='default', help='AWS profile name (default: %(default)s)'
    )
    parser.add_argument(
        '--region', default=DEFAULT_REGION, help='AWS region (default: %(default)s)'
    )
    parser.add_argument(
        '--instance-name', default=DEFAULT_INSTANCE_NAME, help='EC2 instance name'
    )
    parser.add_argument(
    ...
    parser.add_argument(
        '--key-name', default=DEFAULT_KEY_NAME, help='EC2 key pair name'
    )
    parser.add_argument(
        '--tag-key', default=DEFAULT_TAG_KEY, help='Tag key for instance resources'
    )
    parser.add_argument(
        '--tag-value',
        default=DEFAULT_TAG_VALUE,
        help='Tag value for instance resources',
    )
    parser.add_argument(
        '--root-volume-size',
        type=int,
        default=DEFAULT_ROOT_VOLUME_SIZE,
        help='Root volume size (GiB)',
    )
    parser.add_argument(
        '--data-volume-size',
        type=int,
        default=DEFAULT_DATA_VOLUME_SIZE,
        help='Data volume size (GiB)',
    )
    parser.add_argument(
        '--ssh-public-key',
