...
def load_config_from_env_and_args() -> Config:
    parser = argparse.ArgumentParser(
        description='Nuke AWS resources in a region (use with caution)'
    )
    parser.add_argument(
        '--region', default=os.getenv('REGION', os.getenv('AWS_REGION', 'us-west-1'))
    )
    ...
    parser.add_argument(
        '--nuke-s3',
        action='store_true',
        default=os.getenv('NUKE_S3', '0') == '1',
        help='Delete ALL S3 buckets (global)',
    )
    parser.add_argument(
        '--wait-timeout',
        type=int,
        default=int(os.getenv('WAIT_TIMEOUT_SECONDS', '180')),
    )
    parser.add_argument(
        '--yes',
        action='store_true',
    ...
    **base_kwargs: Any,
    ...
    try:
        while True:
            ...
            next_token = (
                cast(Optional[str], next_token_value) if next_token_value else None
            )
            if not next_token:
                break
    except ClientError:
...
def list_elbv2_target_group_arns(
    elbv2: ElasticLoadBalancingv2Client, lb_arn: str
) -> list[str]:
...
def list_security_group_ids_for_vpc(
    ec2: EC2Client, vpc_id: str, exclude_default: bool = True
) -> list[str]:
    try:
    ...
    try:
        while True:
            ...
            if resp.get('IsTruncated'):
                ...
                marker = (
                    cast(Optional[str], next_marker_val) if next_marker_val else None
                )
                if marker is None:
                    break
            else:
        ...
        while True:
            ...
            for entry in resp.get('Keys', []) or []:
                ...
                if (
                    ...
                    or meta.get('KeyState') == 'PendingDeletion'
                ):
                    continue
                keys.append(key_id)
...
def discover(clients: AwsClients, nuke_s3: bool) -> Optional[AwsSnapshot]:
    ...
    delete_plan = [
        ...
        (
            'kms_customer_keys',
            'KMS Key (custom, schedule deletion)',
            ' - No custom KMS keys',
        ),
...
def delete_elbs(
    clients: AwsClients, elbv2_arns: list[str], elb_names: list[str]
) -> None:
...
def report_leftovers(clients: AwsClients, nuke_s3: bool) -> None:
    ...
    print(
        f'KMS keys (custom, not pending deletion): {len(list_kms_customer_keys(clients.kms))}'
    )
    if nuke_s3:
...
def _teardown_cluster(clients: AwsClients, cluster_name: str) -> None:
    ...
    for profile in fprofiles:
        ...
        try:
            clients.eks.delete_fargate_profile(
...
def _teardown_vpc_resources(clients: AwsClients, vpc_id: str) -> None:
    ...
    for route_table in route_tables:
        ...
        for route in routes:
            if (
                route.get('Origin') == 'CreateRouteTable'
                or route.get('GatewayId') == 'local'
            ):
                continue
